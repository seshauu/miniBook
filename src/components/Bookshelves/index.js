import {Component} from 'react'

import Cookies from 'js-cookie'
import Loader from 'react-loader-spinner'

import {BsSearch} from 'react-icons/bs'

import Header from '../Header'
import Footer from '../Footer'
import BookItem from '../BookItem'
import './index.css'

const bookshelvesList = [
  {
    id: '22526c8e-680e-4419-a041-b05cc239ece4',
    value: 'ALL',
    label: 'All',
  },
  {
    id: '37e09397-fab2-46f4-9b9a-66b2324b2e22',
    value: 'READ',
    label: 'Read',
  },
  {
    id: '2ab42512-3d05-4fba-8191-5122175b154e',
    value: 'CURRENTLY_READING',
    label: 'Currently Reading',
  },
  {
    id: '361d5fd4-9ea1-4e0c-bd47-da2682a5b7c8',
    value: 'WANT_TO_READ',
    label: 'Want to Read',
  },
]

const apiStatus = {
  initial: 'INITIAL',
  success: 'SUCCESS',
  failure: 'FAILURE',
  inProgress: 'IN_PROGRESS',
}

class Bookshelves extends Component {
  state = {
    activeFilter: bookshelvesList[0].value,
    booksApiStatus: apiStatus.initial,
    booksData: {},
    searchInput: '',
    search: '',
    activeFilterLabel: bookshelvesList[0].label,
  }

  componentDidMount() {
    this.getBookApiData()
  }

  updateBooksList = booksList =>
    booksList.map(each => ({
      id: each.id,
      title: each.title,
      readStatus: each.read_status,
      rating: each.rating,
      authorName: each.author_name,
      coverPic: each.cover_pic,
    }))

  getBookApiData = async () => {
    this.setState({booksApiStatus: apiStatus.inProgress})
    const {search, activeFilter} = this.state
    const booksApi = ` https://apis.ccbp.in/book-hub/books?shelf=${activeFilter}&search=${search}`
    const jwtToken = Cookies.get('jwt_token')
    const options = {
      method: 'GET',
      headers: {
        Authorization: `Bearer ${jwtToken}`,
      },
    }
    const response = await fetch(booksApi, options)
    if (response.ok === true) {
      const fetchedData = await response.json()
      const updatedData = {
        books: this.updateBooksList(fetchedData.books),
        total: fetchedData.total,
      }
      this.setState({
        booksData: updatedData,
        booksApiStatus: apiStatus.success,
      })
    } else {
      this.setState({booksApiStatus: apiStatus.failure})
    }
  }

  onClickRetry = () => {
    this.getBookApiData()
  }

  onChangeInput = event => {
    this.setState({searchInput: event.target.value})
  }

  onSearchBooks = () => {
    this.setState(
      prevState => ({
        search: prevState.searchInput,
      }),
      this.getBookApiData,
    )
  }

  loaderView = () => (
    <div className="loader-con" testid="loader">
      <Loader type="TailSpin" color="#0284c7" height={32} width={32} />
    </div>
  )

  renderBooksFailureView = () => (
    <div className="failure-con">
      <img className="failure-img" src="" alt="failure view" />
      <p className="failure-heading">Something went wrong. Please try again</p>
      <button className="failure-btn" onClick={this.onClickRetry} type="button">
        Try Again
      </button>
    </div>
  )

  renderListOfBooks = () => {
    const {booksData} = this.state
    const {books} = booksData

    return (
      <ul className="book-list-con">
        {books.map(each => (
          <BookItem key={each.id} bookDetails={each} />
        ))}
      </ul>
    )
  }

  renderNoMatchBooks = () => {
    const {searchInput} = this.state
    return (
      <div className="no-msg-found-con">
        <img
          className="no-match-img"
          src="https://res.cloudinary.com/dw6hgsfbk/image/upload/v1702363049/Asset_1_11px_z9mjnl.png"
          alt="no books"
        />
        <p className="no-match-para">
          Your search for {searchInput} did not find any matches.
        </p>
      </div>
    )
  }

  renderBooksSuccessView = () => {
    const {booksData} = this.state
    const {total} = booksData
    if (total !== 0) {
      return this.renderListOfBooks()
    }
    return this.renderNoMatchBooks()
  }

  renderBooks = () => {
    const {booksApiStatus} = this.state
    switch (booksApiStatus) {
      case apiStatus.success:
        return this.renderBooksSuccessView()
      case apiStatus.failure:
        return this.renderBooksFailureView()
      case apiStatus.inProgress:
        return this.loaderView()
      default:
        return null
    }
  }

  render() {
    const {activeFilter, searchInput, activeFilterLabel} = this.state

    return (
      <>
        <Header />
        <div className="book-shelves-bg-con">
          <div className="filter-container-lg">
            <h1 className="bookshelves-heading bookshelves-heading-lg">
              Bookshelves
            </h1>
            <ul className="filter-list-con-lg">
              {bookshelvesList.map(each => {
                const activeFilterClass =
                  activeFilter === each.value ? 'active-filter-lg' : ''
                const onClickedFilter = () => {
                  this.setState(
                    {
                      activeFilter: each.value,
                      activeFilterLabel: each.label,
                    },
                    this.getBookApiData,
                  )
                }

                return (
                  <li className="active-filter-list-lg" key={each.label}>
                    <button
                      className={`active-filter-btn-lg ${activeFilterClass}`}
                      onClick={onClickedFilter}
                      type="button"
                    >
                      {each.label}
                    </button>
                  </li>
                )
              })}
            </ul>
          </div>
          <div>
            <div className="filter-books-search-con">
              <h1 className="filtered-book-heading">
                {activeFilterLabel} Books
              </h1>
              <div className="search-input-con">
                <input
                  className="search-input"
                  placeholder="Search"
                  type="search"
                  onChange={this.onChangeInput}
                  value={searchInput}
                />
                <button
                  className="search-btn"
                  onClick={this.onSearchBooks}
                  type="button"
                  testid="searchButton"
                >
                  <BsSearch className="search-icon" />
                </button>
              </div>
            </div>
            <h1 className="bookshelves-heading bookshelves-heading-sm">
              Bookshelves
            </h1>
            <div className="books-filter-sm">
              {bookshelvesList.map(each => {
                const activeFilterClass =
                  activeFilter === each.value ? 'active-filter-btn' : ''
                const onClickFilter = () => {
                  this.setState(
                    {
                      activeFilter: each.value,
                      activeFilterLabel: each.label,
                    },
                    this.getBookApiData,
                  )
                }
                return (
                  <button
                    className={`bookshelves-filter-btn ${activeFilterClass}`}
                    type="button"
                    key={each.label}
                    onClick={onClickFilter}
                  >
                    {each.label}
                  </button>
                )
              })}
            </div>
            <div>{this.renderBooks()}</div>
            <Footer />
          </div>
        </div>
      </>
    )
  }
}

export default Bookshelves
